            ...
            <form method="POST" action="{{ route('educational-activities.update', $educationalActivity) }}">
                ...
                <div class="form-group">
                    ...
                    <select name="type" id="type" required>
                        <option value="Prednáška" {{ $educationalActivity->type == 'Prednáška' ? 'selected' : '' }}>
                            Prednáška
                        </option>
                        <option value="Cvičenie" {{ $educationalActivity->type == 'Cvičenie' ? 'selected' : '' }}>
                            Cvičenie
                        </option>
                        <option value="Skúška" {{ $educationalActivity->type == 'Skúška' ? 'selected' : '' }}>Skúška
                        </option>
                        <option value="Iné" {{ $educationalActivity->type == 'Iné' ? 'selected' : '' }}>Iné</option>
                    </select>
                </div>

                <div class="form-group">
                    <label for="duration">Dĺžka trvania (minúty):</label>
                    <input type="number" name="duration" id="duration" value="{{ $educationalActivity->duration }}"
                           required>
                </div>
                ...
                <div class="form-group">
                    ...
                    <select name="repetition" id="repetition" required onchange="checkRepetition(this.value)">
                        <option value="Každý" {{ $educationalActivity->repetition == 'Každý' ? 'selected' : '' }}>Každý
                        </option>
                        <option value="Párny" {{ $educationalActivity->repetition == 'Párny' ? 'selected' : '' }}>Párny
                        </option>
                        <option value="Nepárny" {{ $educationalActivity->repetition == 'Nepárny' ? 'selected' : '' }}>
                            Nepárny
                        </option>
                        <option
                ...
                <div class="form-group" id="eventDateGroup" style="display: none;">
                    ...
                    <input type="date" name="event_date" id="event_date" value="{{ $educationalActivity->event_date }}"
                ...
                @foreach (['Po', 'Ut', 'St', 'Št', 'Pi'] as $day)
                    <div class="form-group">
                        ...
                        <input type="time" name="preferred_time[{{ $day }}][start]" id="preferred_time_{{ $day }}_start"
                               value="{{ $preferredDayTime[$day]['start'] ?? '' }}" min="07:00" max="20:00">

                        <label for="preferred_time_{{ $day }}_end">Do:</label>
                        <input type="time" name="preferred_time[{{ $day }}][end]" id="preferred_time_{{ $day }}_end"
